...
author: "Aida Aquino Arias, Jacob Organista"
format: html
...
military <- read_xlsx("data/gov_spending_per_capita.xlsx",  #want to read and change the specified data of gov-spending-per-capita
                      sheet = "Share of Govt. spending",    #specifying that we want to use the share of Govt. spending
                      skip  = 7,                            #specifying that we want to skip 7 rows before reading in the data
                      n_max = 191,                          #specifying that we want to read in max 191 rows of data
                      na = c("xxx", ". ."))                  #specify that the "xxx" and ".." would be read as NA
```
...
> The specific line of code filter(!Country %in% regions) filters out rows from the data, and it will only include rows where Country does not match the specifies region.
...
military_long |> 
  ...
  ggplot(aes(x = spending, y = Country)) +
  
  geom_boxplot() +
  labs(
    x = "Military Spending Total %",
    y = "Eastern Europe Countries"
  ) +
  theme_minimal()
